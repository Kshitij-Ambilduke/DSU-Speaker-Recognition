...
class SpeakerLSTM(nn.Module):
    ...
    def forward(self, audios, lengths, labels):
        ...
        (all_hidden, lengths) = pad_packed_sequence(packed_output, batch_first=True)

        batch_indices = torch.arange(len(lengths)).to(DEVICE)
...
def train(model, train_dataloader, val_dataloader, criterion, epochs, optimizer, batch_size=2, clip=1.0):
    ...
    if best_model:
        torch.save(best_model, "/mnt/data-artemis/kshitij/TEMP/zsaved_things/model/speaker_lstm_best_LS_DSU_dedup.pth")
...
if __name__ == '__main__':
    ...
    model = SpeakerLSTM(num_layers=4, bidirectional=True, input_dim=128, hidden_dim=128).to(DEVICE)
    print(model)
    epochs = 100
    LR = 0.001
    batch_size = 512
    ...
    val_loader = get_loader(batch_size=1, 
                              df_path="/mnt/data-artemis/kshitij/TEMP/Speaker-recognition/DSU_creation/LibriSpeech/final_data_librispeech/dev.csv",
                              mapping_path="/mnt/data-artemis/kshitij/TEMP/Speaker-recognition/DSU_creation/LibriSpeech/speaker_label_mapping.json" )
    ...
    torch.save(model.state_dict(), "/mnt/data-artemis/kshitij/TEMP/speaker_lstm_vctk_DSU_Dedup.pth")

    save_path = "/mnt/data-artemis/kshitij/TEMP/zsaved_things/plots/speaker_lstm_LS_DSU_dedup.png"
